...
export default function App() {
  ...
  const { votes, customGames, loading, tally, totalVoters, submitVote, getExistingVote, addCustomGame, resetAll } = usePoll()

  const allDefaultGames = DEFAULT_GAMES.flatMap(c => c.games)

  // Load previous vote when name changes (on blur)
  const handleNameBlur = async () => {
    const trimmed = name.trim()
    if (!trimmed) return
    const prev = await getExistingVote(trimmed)
    if (prev?.games) {
      setSelected(new Set(prev.games))
      setHasVoted(true)
      showStatus('► Vorige stem geladen — pas aan en sla opnieuw op', 'info')
    }
  }
  ...
  const handleReset = async () => {
    if (!confirm('Alle stemmen én toegevoegde games verwijderen?')) return
    await resetAll()
    setSelected(new Set())
    setHasVoted(false)
    setName('')
